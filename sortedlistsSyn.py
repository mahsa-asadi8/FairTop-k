...
numberofSample = 30000
print("dataset size:", numberofSample)


'''
instance1 = random.sample(range(800000, 1000000), 30)

instance2 = random.sample(range(1000, 100000), 9970)

instance = np.concatenate((instance1, instance2), axis=None)
'''
# IMDB
'''
...
for i in range(0, len(instance)):
    for j in range(i + 1, len(instance)):
        if i != j:
            ...
            #dist = round(dist, 2)
            dist = manhattan(instance[i], instance[j])
            pairs = [indexMap[instance[i]], indexMap[instance[j]]]
            pairs.sort()
            tup = tuple(pairs)
...
'''
file_to_write = open("sortedRel30k-airbnb.pickle", "wb")

pickle.dump(sorted_rel, file_to_write)

#file_to_write2 = open("sortedDiv10k-synthetic.pickle", "wb")
